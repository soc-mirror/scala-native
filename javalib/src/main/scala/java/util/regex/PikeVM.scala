...
object PikeVM {
  ...
  private val lineTerminator: CharacterMatcher =
...
class PikeVM protected[regex] (val program: Array[Int],
                               val findPrefixLength: Int,
                               val groupCount: Int,
                               val classes: Array[CharacterMatcher],
                               val lookarounds: Array[PikeVM]) {
  ...
  private class ThreadQueue() {
    ...
    private var offsets: Array[Array[Int]] =
      new Array[Array[Int]](program.length + 1)
     ...
     */
    def queueImmediately(currentPC: Int,
                         nextPC: Int,
                         copyThreadState: Boolean): Boolean = {
     ...
     *         same { @code nextPC} already)
     */
    private[regex] def queueNext(currentPC: Int,
                                 nextPC: Int,
                                 next: PikeVM#ThreadQueue): Boolean = {
      if (next.tail < 0) {
        next.head = nextPC
          ...
          def _step(): Boolean = {
            ...
            opcode match {
              ...
              case WORD_BOUNDARY | NON_WORD_BOUNDARY => {
                val i2: Int = i - step
                val c2: Int =
                  if (i2 < 0 || i2 >= characters.length) -1
                  else characters(i2)
                opcode match {
                  case WORD_BOUNDARY =>
                    ...
                    } else if (i >= 0 && i < characters.length && !PikeVM.wordCharacter
                                 .matches(c)) {
                      current.queueImmediately(pc, pc + 1, false)
                    }
                  case NON_WORD_BOUNDARY =>
                    if (c2 < 0 || !PikeVM.wordCharacter.matches(c2.toChar)) {
                      if (i >= 0 && i < characters.length && !PikeVM.wordCharacter
                            .matches(c)) {
                        current.queueImmediately(pc, pc + 1, false)
                      }
  ...
  def isPlainString: String = {
    ...
    if (start + 1 < program.length && program(start) == SAVE_OFFSET && program(
            start + 1) == 0) {
      start += 2
    }
