...
/**
  ...
  */
class RegexPattern private[regex](val regex: String, override val flags: Int, var vm: PikeVM) extends Pattern(regex, flags) {
  override def matcher(string: CharSequence): RegexMatcher = new RegexMatcher(vm, string)
}